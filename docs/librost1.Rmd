...
El resumen estadístico del modelo se imprime con la función _summary()_
...
Una manera mucho mas atractiva de mostrar los resultados es por medio de la función _autoplot()_ de la librería _forecast_ y las opciones gráficas de la librería _ggplot2_.
...
Del modelo se desprende que, _caeteris paribus_, que el incremento mensual del Índice Nacional de Precios al Consumidor es de `r b` y cuenta con un intercepto de `r a`


### Tendencia cuadrática

Se puede modelar una tendencia que, en principio, parece ser cuadrática. La variable en cuestión es la **Tasa de condiciones críticas de ocupación (TCCO)** (Porcentaje), cuyo identificador es 444608. La tasa de condiciones críticas de ocupación representa el segmento de la población económicamente activa que cuenta con alguna de las siguientes características: 
...
Se debe, al igual que el ejercicio anterior sobre tendencia lineal, construir la variable del tiempo. Además, como la variable, aparentemente, muestra una tendencia cuadrática se debe generar el tiempo al cuadrado. Con ambas variables del tiempo se construye un modelo de regresión. Tenemos dos opciones, con la función _lm()_ en la que se incluye, como variables independientes, al tiempo y al tiempo al cuadrado. O por medio de la función _tslm()_ que permite capturar este fenómeno con la instrucción *trend*. 
Al momento de correr el modelo se obtienen los coeficientes estimados, lo cuales permiten construir una variable de tendencia. 

```{r}
tiempo <- time(TCCO)
tiempo2 <- tiempo^2
mod1 <- lm(TCCO~tiempo+tiempo2)
...
Para comparar el ajuste entre el fenómeno observado y la tendencia estimada se corre un _ts.plot()_.

```{r}
ts.plot(TCCO, tendenciaC, col=1:2)
```

Como resultado final, es posible obtener un pronóstico para los siguientes 12 meses, ya que la variable es mensual. 
...
En el caso de una variable con tendencia cúbica, se usa la **Tasa de ocupación parcial y desocupación 1 (TOPD1)** (Porcentaje), con identificador 444604. La tasa de ocupación parcial y desocupación es el porcentaje de la población económicamente activa que se encuentra desocupada, más la ocupada que trabajó menos de 15 horas en la semana.
...
Tasa de presión general (TPRG) (Porcentaje), mide el porcentaje que representa la población desocupada abierta y los ocupados que buscan trabajo con el propósito de cambiarse o tener un empleo adicional con respecto a la Población Económicamente Activa. Su identificador en el BIE es 444605. Esta es una variable que, dadas sus características, se puede modelar con una tendencia polinomial. 
...
La recaudación de impuestos a nivel nacional identificada como parte de los ingresos y egresos brutos de las finanzas públicas se puede modelar de forma exponencial. Su ubicación en el BIE es Finanzas públicas > Ingresos y egresos brutos por entidad federativa > Nacional 31 entidades federativas (no incluye Ciudad de México) > Ingresos > Impuestos y cuenta con la clave: 441566.
...
Los incrementos en número de contagios de covid-19, a nivel global, sugieren que el patrón subyacente se comporta como algún proceso de crecimiento poblacional. Estos procesos, aunque son muy recurrentes en sistemas complejos aplicados a curvas de aprendizaje y, fundamentalmente, en un gran número de procesos naturales, se caracterizan por seguir una progresión bien definida con valores relativamente bajos al inicio, muy acelerados en la parte intermedia, y llegando a un clímax en la parte alta. La característica fundamental de este tipo de modelos es que muestran un comportamiento gráfico similar a una “S”, por lo que se identifican como funciones sigmoides y su función más conocida es la función logística, la cual se debe al aporte de Verhulst (1938) quien potenció el análisis poblacional de Malthus. Sin embargo, la función logística tiene dos características sumamente restrictivas: la primera es que, empíricamente, se ha observado que existen procesos que no alcanzan su mayor tasa de crecimiento justo a la mitad del periodo de crecimiento; segundo, derivado de la observación anterior, momento en el que se espera el punto de inflexión no corresponde con la mitad del tiempo de reproducción. 

Por ello, es necesario el uso de una función más general que el caso simple de Verhulst. La función que se emplea es, al igual que la logística, una función sigmoide de tipo Gompertz, la cual  se debe a Benjamin Gompertz (1925) quien publicó un artículo titulado _"On the Nature of the Function Expressive of the Law of Human Mortality"_. Este tipo de función cuenta con 3 bondades sobre la de Verhulst: 1) es un proceso aplicado, originalmente, a las tasas de mortalidad humana; 2) pertenece a la familia de funciones logísticas generalizadas o curvas de Richards, siendo un caso particular de ellas y; 3) en términos empíricos es mucho más eficiente pues los parámetros permiten una gran versatilidad. El modelo de crecimiento de Gompertz ha sido ampliamente utilizado en crecimiento poblacional de ciudades, en biología ha sido empleado en crecimiento poblacional de animales y plantas, y en estimaciones del volumen de ciertas bacterias y células cancerígenas. 

El proceso subyacente de los contagios por covid-19 en México, ya que es un virus que se contagia entre humanos, se modela como una curva de crecimiento tipo Gompertz de la siguiente manera:

$$y_t=Ae^{-e^{-\left(\frac{t-b}{c}\right)}} $$

donde $Y=$ número de contagios
	    $A=$ parámetro de valor asintótico
	    $b=$ parámetro de desplazamiento de la curva
	    $c=$ parámetro de la tasa de crecimiento
	    $t=$ tiempo
...
La estimación de los parámetros A,b y c se realiza por medio de un modelo de regresión de mínimos cuadrados no lineales. El modelo, al estimarse por medio de un proceso iterativo de convergencia con ajuste paramétrico da como resultado:
...
El segundo componente subyacente es la estacionalidad. La estacionalidad es el comportamiento de corto plazo que se puede atribuir a momentos específicos y es facilmente identificable. Normalmente se aprecia en datos con temporalidad menor a un año.

Existen dos tipos de estacionalidad, la aditiva y la multiplicativa. 

### Estacionalidad aditiva

Comportamiento que se presenta de manera regular y sistemática con impulsos de la misma, o similar, margnitud. 
En este caso, el índice de volumen físico de la cría y explotación de animales como indicador global de la actividad económica de actividades primarias nos permite evaluar la presencia de estacionalidad aditiva. El identificador de la variable en el BIE es: 737124
...
Como se puede apreciar en el gráfico anterior, hay picos y valles que se presentan de manera regular a lo largo del tiempo en el que se captura la variable. Ese es el primer indicio de la presencia de estacionalidad. Para corroborar su presencia es necesario emplear algunos otros elementos gráficos de análisis. 

El gráfico estacional o _Seasonal Plot_ nos permite evaluar, al interior de un año, el efecto mensual de las estaciones que se presentan a lo largo del año. Si no hubiera efecto estacional no se apreciarían picos regulares en los mismos meses. 
...
Otra herramienta similar el esl gráfico polar estacional o _Polar Seasonal Plot_, a diferencia del anterior, si no hubiera estacionalidad entonces se apreciaría un espiral concéntrico. En la medida en la que e espiral se deforma en meses específicos, es un indicio de que en dicho mes se da un efecto estacional. 
...
EL gráfico de subseries estacionales o _Subseries Plot_ permite identificar los meses en los que se dá el mayor impacto estacional.
...
El gráfico de rezagos estacionales o _Lag plots_ es muy útil para identificar la fuerza de la estacionalidad. Los colores indican el mes en el eje vertical. Las líneas conectan puntos en orden cronológico. La relación es fuertemente positiva en los rezagos en los que se alejan de la recta de 45°.
...
La manera adecuada de incorporar el efecto estacional es por medio de la modeliazación a traves de variables dicotómicas que capturen los periodos reflejados en la temporalidad de la variable. Por ejemplo, si la variable tiene una temporalidad mensual, entonces se deben construir variables cualitativas que midan la presencia de cada uno de los meses. 

Así, la variable dicotómica o variable dummy toma dos valores, 0 y 1. 1 cuando el dato se presenta en el mes indicado y 0 en cualquier otro caso. 

$$D_s \in\{0,1\} $$
El componente estacional $S_t$ se incorpora por medio de la adición de los elementos al modelo general. 

$$ y_t= T_t+S_t+C_t+I_t$$

En particular, el componente estacional estima, por medio del modelo econométrico, los coeficientes asociados a cada observación estacional. 

$$S_t = \delta_1 D_1 + \delta_2D_2+...+\delta_sD_s$$
tommando como s al numero de periodos observados en la temporalidad dentro de un año. 

Así para un modelo con estacionalidad se tiene que  

$$y_t= S_t = \delta_1 D_1 + \delta_2D_2+...+\delta_sD_s$$
$$y_t= S_t = \Sigma_i^s\delta_i D_i$$

La incorporación de las variables dummy se consigue por medio de la función _seasonaldummy()_ perteneciente a la librería *forecast*
...
Si se construye un modelo de regresión que incluya las variables estacionales.  El modelo resutante estima, para el caso del Índice de Volumen Físico de actividades primarias en cría y explotación de animales la temporalidad es mensual, por lo que se estiman 11 coefiecientes asociados con cada mes de observación y el intercepto. Como podrá recordar recordar de cursos prevíos de econometría, el uso de variables dummy requiere eliminar de la estimación al menos una de las variables cualitativas para evitar problemas de multicolinealidad perfecta. 

```{r}
m6 <- lm(IVFEA~dummies)
```

Con esto se consigue la estimación que cada mes le imprime al fenómeno en cuestión. La represenación se reproduce en el siguiente gráfico: 
...
Al gráficar el valor estimado y el observado  se aprecia que al valor estimado no se ajusta al valor observdo. El modelo está mal especificado, pues le falta la incorporación de la tendencia. 

```{r}
ts.plot(IVFEA, m6$fitted.values, col=1:2)
```

Si se incluye una tendnecia, en principio, lineal el valor estimado se ajusta mucho mejor al observado, lo que sugiere que este variable se debe modelar con tendencia y estacionalidad aditiva. Acontinuación se muestra una manera alterna de modelar este fenómeno. Se sustituye la función _lm()_ por _tslm()_. Está función permite modelar de manera directa los componentes de tendencia y estacionalidad y permite ajustar de manera directa el pronóstico.  
...
Con esto, se consigue un pronóstico que, además de capturar el fenómeno estacional y tendencial, muestra intervalos de confianza muy estrechos. 
...
### Estacionalidad multiplicativa
 ...
 El otro tipo de estacionalidad es la multiplicativa. Esta, a diferencia de la aditiva, muestra un comportamiento creciente a lo largo del tiempo. 
 
 Para representar el fenómeno se emplea el indicador global de la actividad económica para actividades primarias. El índice de volumen físico de la agricultura con identificador: 737123.
...
Si se procede de la misma manera que en el caso anterior, el resultado es el siguiente:
...
Como se puede apreciar, el ajuste no es el adecuado, al inicio del fenómeno se sobre estima la variable y en la parte final se subestima, justo porque no se captura el efecto multiplicativo de la estacionalidad. 

```{r}
ts.plot(IVFAgr, mod7$fitted.values, col=1:2)
```

Lo adecuado es modelar el efecto multiplicativo con la transformación adecuada. 

$$y_t = T_t*S_t*I_t $$
$$ln(y_t)=T_t+S_t+I_t $$

Por lo tanto, la estimación queda de la siguiente manera:
...
EL ajuste entre la variable observada y estimada queda como sigue:
